...
static const mbedtls_md_info_t kSha256Info = {1};

const mbedtls_md_info_t *mbedtls_md_info_from_type(mbedtls_md_type_t type)
{
	return (type == MBEDTLS_MD_SHA256) ? &kSha256Info : NULL;
}
...
void sha256(const unsigned char *data, size_t len, unsigned char out[32])
{
	...
	auto process = [&](const unsigned char *chunk) {
		uint32_t w[64];
		for (int i = 0; i < 16; ++i) {
