...
- **LoRa / Meshtastic UART (Urban C6)** — Prototype DM over CN4 UART (**TX GPIO22 / RX GPIO20**, **115200**, Heltec **Serial Module = PROTO**). Phone API session (`want_config_id`, heartbeat) plus **PKI unicast** to a configured destination (`!xxxxxxxx`), **PortNum 256**. Payload is **`core.v1.Message`** (not `SignedEnvelope`). Transport v1: `SINGLE` (`0x01`) if ≤218 bytes, else `FRAGMENT` (`0x41`, SHA-256[0:6], 211-byte chunks). Interval default **180 s**. USB CDC keeps firmware logs.
