...
- **LoRa / Meshtastic UART (Urban C6)** — Optional compact JSON Lines telemetry on CN4: **TX GPIO22** (pin 6), **RX GPIO20** (pin 8), **115200** baud. Off by default (`System → Send compact JSON to LoRa UART`). After enable, UART0 is remapped to those pins so Meshtastic sees sensor JSON only; USB CDC keeps firmware logs. Payload is one signed JSON line (`id`, `ts`, `p1`/`p2`, `t`/`h`/`p`, `n`/`nm`, `s`), max **237** bytes (+ CRLF on the wire). `s` is standard base64 of an Ed25519 signature over the same object **without** `s` (stable key order; same device key as the map). Unsigned lines are not sent. Send period is independent of the map interval (`LoRa sending interval`, default **180 s**, floor **30 s**). Heltec Serial Module: enabled, **TEXTMSG**, baud enum **11**, timeout **1 s**; cross TX/RX and share GND.
